...
public final class TabAPI implements Listener
{
	 ...
	 * <p>
	 * If the plugin the tab is being set from does not have a priority, it will automatically be give a base priority of 0.
	 * 
	 * @param player
	 * @param x
	 * @param y
	...
	public void updatePlayer(Player player)
	{
		...
		for (int i = 0; i < holder.maxv; i++)
		{
			for (int j = 0; j < holder.maxh; j++)
			{
				...
				String truncatedMsg = msg.substring(0,
	...
	public void clearTab(Player player)
	{
		...
		for (String[] row : holder.tabs)
		{
			for (String msg : row)
			{
				...
				addPacket(player, msg.substring(0, Math.min(msg.length(), 16)),
						false, 0);
			}
		}
	}
	...
	private static final String[] colors = { "1", "2", "3", "4", "5", "6", "7",
			"8", "9", "0", "a", "b", "c", "d", "f", "g", "h", "i", "j", "k",
