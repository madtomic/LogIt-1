...
public final class ConfigurationManager extends LogItCoreObject
{
	...
	public void registerConfiguration(String filename, String userConfigDef,
			String packageConfigDef, String header)
	{
		if (StringUtils.isBlank(filename) || userConfigDef == null
				|| packageConfigDef == null)
		{
			throw new IllegalArgumentException();
		}

		if (registrations.containsKey(filename))
			throw new RuntimeException("Configuration already registered: "
					+ filename);

		registrations.put(filename, new PredefinedConfiguration(filename,
				userConfigDef, packageConfigDef, header));
	}
