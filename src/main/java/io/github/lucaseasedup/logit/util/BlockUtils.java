...
public final class BlockUtils
{
	...
	{
		return material != Material.PORTAL && material != Material.LAVA
				&& material != Material.WATER && material != Material.AIR
	...
	public static boolean isBlockAdjacent(Block block, BlockFace face,
	 ...
	 * <p>
	 * {@code block} is considered safe to stand on if {@code BlockUtils.isMaterialSafe(block.getType()) == true}.
	 * 
	 * @param origin
	 *            the block around which search will start.
	 * @param maxRadius
	...
	public static Location getNearestSafeSpace(Location origin, int maxRadius)
	{
		...
		BlockFace[][] orths = { { BlockFace.NORTH, BlockFace.EAST },
		...
		for (int r = 0; r <= maxRadius; r++)
		{
			for (int s = 0; s < 6; s++)
			{
				...
				for (int x = -r; x <= r; x++)
				{
					for (int y = -r; y <= r; y++)
					{
						Block block = referenceBlock.getRelative(orth[0], x)
