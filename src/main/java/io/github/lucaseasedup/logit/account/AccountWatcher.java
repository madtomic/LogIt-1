...
public final class AccountWatcher extends LogItCoreObject implements Runnable
{
	...
	public void run()
	{
		boolean accountDeletionEnabled = getConfig("config.yml").getBoolean(
				"automaticAccountDeletion.enabled");

		if (!accountDeletionEnabled)
			return;

		long now = System.currentTimeMillis() / 1000L;
		long inactivityTime = getConfig("config.yml").getTime(
				"automaticAccountDeletion.inactivityTime", TimeUnit.SECONDS);

		List<Account> accounts = getAccountManager().selectAccounts(
				Arrays.asList(keys().username(), keys().last_active_date()),
				new SelectorBinary(new SelectorCondition(keys()
						.last_active_date(), Infix.GREATER_THAN, "0"),
						Infix.AND, new SelectorNegation(new SelectorCondition(
								keys().last_active_date(), Infix.GREATER_THAN,
		...
		if (!accounts.isEmpty())
		{
			...
			getAccountManager()
					.removeAccounts(
							accountsToDelete
									.toArray(new String[accountsToDelete.size()]));
		}
	}
	 ...
	 */
	public static final long TASK_PERIOD = TimeUnit.MINUTES.convert(10,
			TimeUnit.TICKS);
}
