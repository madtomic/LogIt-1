		...
		CommandExecutor
{
			...
			String[] args)
	{
		...
		if (args.length < 3)
		{
			...
			{
				if (player != null
						&& !player.hasPermission("logit.loginhistory.others"))
				{
					sendMsg(sender, t("noPerms"));
			...
			Account account = getAccountManager().selectAccount(playerName,
					Arrays.asList(keys().username(), keys().login_history()));
				...
				else
				{
					sendMsg(sender,
							t("notRegistered.others").replace("{0}",
									PlayerUtils.getPlayerRealName(playerName)));
				}
			...
			for (int i = 0, n = records.size(); i < n - 1; i++)
			{
				...
				if (i + 1 >= n - 1)
				{
					printLoginRecord(sender, nextSplit[0], nextSplit[1],
							nextSplit[2], equalRecords, split[1]);
				}
	...
	private void printLoginRecord(CommandSender sender, String unixTime,
			String ip, String succeeded, int equalRecords, String lastIp)
	{
		...
		if (equalRecords > 1)
		{
			repetition = t("loginHistory.record.repetition").replace("{0}",
					String.valueOf(equalRecords));
		}
		else
		{
			repetition = "";
		...
		sendMsg(sender,
				t(messageLabel)
						.replace(
								"{0}",
								new Date(Long.parseLong(unixTime) * 1000L)
										.toString()).replace("{1}", ip)
