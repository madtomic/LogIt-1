...
public final class AccountInfoHubCommand extends HubCommand
{
	public AccountInfoHubCommand()
	{
		super("account info", new String[] { "username" },
				new CommandAccess.Builder().permission("logit.account.info")
						.playerOnly(false).runningCoreRequired(true).build(),
				new CommandHelpLine.Builder().command("logit account info")
						.descriptionLabel("subCmdDesc.account.info").build());
	}
	...
	public void execute(CommandSender sender, String[] args)
	{
		Account account = getAccountManager().selectAccount(
				args[0],
				Arrays.asList(keys().username(), keys().ip(), keys().email(),
						keys().last_active_date(), keys().reg_date(), keys()
								.is_locked(), keys().display_name()));

		if (account == null)
		{
			sendMsg(sender,
					t("notRegistered.others").replace("{0}",
							PlayerUtils.getPlayerRealName(args[0])));
		...
		sendMsg(sender,
				t("accountInfo.username").replace("{0}", account.getUsername()));

		if (!(sender instanceof Player)
				|| sender.hasPermission("logit.account.info.ip"))
		{
			sendMsg(sender, t("accountInfo.ip").replace("{0}", account.getIp()));
		}
		...
		{
			sendMsg(sender,
					t("accountInfo.email").replace("{0}", account.getEmail()));
		}
				...
				|| sender.hasPermission("logit.account.info.lastactivedate"))
		{
			...
			{
				sendMsg(sender,
						t("accountInfo.lastActiveDate").replace(
								"{0}",
								new Date(account.getLastActiveDate() * 1000L)
										.toString()));
			}
		}
				...
				|| sender.hasPermission("logit.account.info.regdate"))
		{
			...
			{
				sendMsg(sender,
						t("accountInfo.registrationDate").replace(
								"{0}",
								new Date(account.getRegistrationDate() * 1000L)
										.toString()));
			}
		}
		...
		{
			sendMsg(sender,
					t("accountInfo.displayName").replace("{0}",
				...
				|| sender.hasPermission("logit.account.info.islocked"))
		{
			if (account.isLocked())
			{
				sendMsg(sender, "");
				sendMsg(sender,
						t("accountInfo.locked").replace("{0}",
								account.getUsername()));
			}
		}
