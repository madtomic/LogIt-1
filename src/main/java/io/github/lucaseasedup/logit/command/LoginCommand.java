		...
		CommandExecutor
{
			...
			String[] args)
	{
		...
		if (args.length > 0 && args[0].equals("-x") && args.length <= 2)
		{
			if (player != null
					&& ((getCore().isPlayerForcedToLogIn(player) && !getSessionManager()
							...
							.hasPermission("logit.login.others")))
			{
			...
			{
				sendMsg(sender,
						t("alreadyLoggedIn.others").replace("{0}",
								paramPlayer.getName()));
			...
			if (!getSessionManager().startSession(paramPlayer).isCancelled())
			{
				...
				sendMsg(sender,
						t("startSession.success.others").replace("{0}",
								paramPlayer.getName()));
				...
				|| (args.length <= 1 && !disablePasswords))
		{
			...
			if (loginBlockade.containsKey(player))
			{
				...
				if (blockadeExpirationTimeMillis - 1000L > System
						.currentTimeMillis())
				{
					long blockageTimeSecs = TimeUnit.MILLISECONDS.convert(
							blockadeExpirationTimeMillis
									- System.currentTimeMillis(),
							TimeUnit.SECONDS);

					sendMsg(player,
							t("tooManyLoginFails.blockLoggingIn").replace(
									"{0}",
									locale.stringifySeconds(blockageTimeSecs)));
			...
			Account account = getAccountManager()
					.selectAccount(
							player.getName(),
							Arrays.asList(keys().username(), keys().salt(),
									keys().password(), keys()
											.hashing_algorithm(), keys().ip(),
									keys().login_history(), keys()
											.persistence()));
					...
					&& !getGlobalPasswordManager().checkPassword(args[0]))
			{
				if (!account.checkPassword(args[0]))
				{
					...
					int failsToBlockLoggingIn = getConfig("config.yml").getInt(
							"bruteForce.blockLogin.attempts");

					int failsToKick = getConfig("config.yml").getInt(
							"bruteForce.kick.attempts");

					int failsToBan = getConfig("config.yml").getInt(
							"bruteForce.ban.attempts");

					Integer currentFailedLogins = failedLogins.get(player);

					failedLogins
							.put(player,
									currentFailedLogins != null ? currentFailedLogins + 1
											: 1);
							...
							&& failedLogins.get(player) >= failsToBlockLoggingIn)
					{
						...
						long loginBlockadeTimeSecs = TimeUnit.MILLISECONDS
								.convert(loginBlockadeTimeMillis,
										TimeUnit.SECONDS);

						Locale locale = getLocaleManager().getActiveLocale();

						loginBlockade.put(player, System.currentTimeMillis()
								+ loginBlockadeTimeMillis);

						sendMsg(player,
								t("tooManyLoginFails.blockLoggingIn")
										.replace(
												"{0}",
					...
					if (getConfig("config.yml").getBoolean(
							"loginHistory.enabled"))
					{
						account.recordLogin(currentTimeSecs, playerIp,
								Account.LOGIN_FAIL);
					}
	...
	private final Map<Player, Integer> failedLogins = PlayerCollections
			.monitoredMap(new HashMap<Player, Integer>());
	private final Map<Player, Long> loginBlockade = PlayerCollections
			.monitoredMap(new HashMap<Player, Long>());
}
