...
public final class ChangePassCommand extends BukkitLogItCoreObject implements
		TabExecutor
{
	@Override
			...
			String[] args)
	{
		...
		if (args.length > 0 && args[0].equals("-x") && args.length <= 3)
		{
			...
			if (args.length < 3)
			{
				sendMsg(sender, t("paramMissing").replace("{0}", "newpassword"));
			...
			sendMsg(sender,
					t("changePassword.success.others").replace("{0}",
							PlayerUtils.getPlayerRealName(args[1])));
		...
		else if (args.length <= 3)
		{
			...
			if (args.length < 1)
			{
				sendMsg(player, t("paramMissing").replace("{0}", "oldpassword"));
			...
			if (args.length < 2)
			{
				sendMsg(player, t("paramMissing").replace("{0}", "newpassword"));
			...
			Account account = getAccountManager().selectAccount(
					...
					Arrays.asList(keys().username(), keys().salt(), keys()
							.password(), keys().hashing_algorithm()));
