...
public final class ProfileEditWizard extends Wizard
{
	...
	protected void onMessage(String message)
	{
		...
		else if (getCurrentStep() == Step.EDIT_FIELD
				|| getCurrentStep() == Step.ERASE_FIELD)
		{
			int number;
			...
			{
				getProfileManager().removeProfileObject(playerName,
						field.getName());
		...
		else if (getCurrentStep() == Step.ENTER_FIELD_VALUE)
		{
			if (field instanceof StringField)
			{
				...
				{
					sendMessage(t("profile.edit.stringTooShort").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(stringField.getMinLength())));
				...
				{
					sendMessage(t("profile.edit.stringTooLong").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(stringField.getMaxLength())));
			...
			else if (field instanceof IntegerField)
			{
				...
				{
					sendMessage(t("profile.edit.invalidValue").replace("{0}",
							message));
				...
				{
					sendMessage(t("profile.edit.numberTooSmall").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(integerField.getMinValue())));
				...
				{
					sendMessage(t("profile.edit.numberTooBig").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(integerField.getMaxValue())));

					return;
				}

				getProfileManager().setProfileInteger(playerName,
						field.getName(), value);
			}
			else if (field instanceof FloatField)
			{
				...
				if (value < floatField.getMinValue())
				{
					sendMessage(t("profile.edit.numberTooSmall").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(floatField.getMinValue())));
				...
				if (value > floatField.getMaxValue())
				{
					sendMessage(t("profile.edit.numberTooBig").replace("{0}",
							field.getName()).replace("{1}",
							String.valueOf(floatField.getMaxValue())));

					return;
				}

				getProfileManager().setProfileFloat(playerName,
						field.getName(), value);
			}
	...
	{
		sendMessage("");
		sendMessage(t("profile.edit.header").replace("{0}", playerName));
		sendMessage(t("wizard.orangeHorizontalLine"));
	...
	private void sendFieldEditingPrompt()
	{
		if (field instanceof StringField)
		{
			sendMessage(t("profile.edit.enterFieldValue.string").replace("{0}",
					field.getName()));
		}
		else if (field instanceof IntegerField)
		{
			sendMessage(t("profile.edit.enterFieldValue.integer").replace(
					"{0}", field.getName()));
		}
		else if (field instanceof FloatField)
		{
			sendMessage(t("profile.edit.enterFieldValue.float").replace("{0}",
					field.getName()));
		}
		else if (field instanceof SetField)
		{
			...
			sendMessage(t("profile.edit.enterFieldValue.set").replace("{0}",
					field.getName()).replace("{1}", values.toString()));
		}
		else
		{
			throw new RuntimeException("Unknown field type: "
