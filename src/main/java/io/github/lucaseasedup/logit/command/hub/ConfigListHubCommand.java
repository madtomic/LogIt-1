...
public final class ConfigListHubCommand extends HubCommand
{
	...
	{
		super("config list", new String[] {}, new CommandAccess.Builder()
				.permission("logit.config.list").playerOnly(false)
				.runningCoreRequired(true).build(),
				new CommandHelpLine.Builder().command("logit config list")
						.descriptionLabel("subCmdDesc.config.list")
						.optionalParam("page").build());
	}

	@Override
	public void execute(CommandSender sender, String[] args)
	{
		Map<String, Property> properties = getConfig("config.yml")
				.getProperties();
		...
		if (args.length > 0)
		{
			...
			{
				sendMsg(sender, t("invalidParam").replace("{0}", "page"));
		...
		sendMsg(sender, "");
		sendMsg(sender,
				t("config.list.header1").replace("{0}", String.valueOf(page))
		...
		{
			if ((i > ((PROPERTIES_PER_PAGE * (page - 1)) - 1))
					&& (j < PROPERTIES_PER_PAGE))
			{
				sendMsg(sender,
						t("config.list.property").replace("{0}",
								e.getValue().getPath()).replace("{1}",
								e.getValue().toString()));
