...
public final class IpcountHubCommand extends HubCommand
{
	...
	{
		super("ipcount", new String[] { "ip" }, new CommandAccess.Builder()
				.permission("logit.ipcount").playerOnly(false)
				.runningCoreRequired(true).build(),
				new CommandHelpLine.Builder().command("logit ipcount")
						.descriptionLabel("subCmdDesc.ipcount").build());
	}
	...
	public void execute(CommandSender sender, String[] args)
	{
		...
		sendMsg(sender,
				t("ipcount").replace("{0}", args[0]).replace(
						"{1}",
