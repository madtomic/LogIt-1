...
public final class BackupRestoreTimeHubCommand extends HubCommand
{
	public BackupRestoreTimeHubCommand()
	{
		super("backup restore time", new String[] { "time" },
				new CommandAccess.Builder().permission("logit.backup.restore")
	...
	public void execute(CommandSender sender, String[] args)
	{
		...
		long desiredDeltaTimeMillis = TimeString.decode(args[0],
		...
		for (File backup : backups)
		{
			try
			{
				Date backupDate = getBackupManager().parseBackupFilename(
						backup.getName());
				long deltaTimeMillis = (currentTimeMillis - backupDate
						.getTime());
				long deltaDifference = Math.abs(desiredDeltaTimeMillis
						- deltaTimeMillis);

				if (closestBackup == null
						|| deltaDifference < smallestDeltaDifference)
				{
		...
		new BackupRestoreFileHubCommand().execute(sender,
				new String[] { closestBackup.getName() });
	}
}
