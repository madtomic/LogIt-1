...
public final class BackupRestoreFileHubCommand extends HubCommand
{
	...
	public void execute(final CommandSender sender, final String[] args)
	{
		...
		{
			sendMsg(sender,
					t("restoreBackup.backupNotFound").replace("{0}", args[0]));
		...
		{
			sendMsg(sender,
					t("restoreBackup.fail").replace("{0}",
							selectedBackup.getName()));
		...
		sendMsg(sender, "");
		sendMsg(sender,
				t("restoreBackup.confirm.filename").replace("{0}",
						selectedBackup.getName()));
		sendMsg(sender,
				t("restoreBackup.confirm.date").replace("{0}",
						selectedBackupDate.toString()));
		sendMsg(sender, "");
		...
		new ConfirmationWizard(sender, "proceed", new Runnable()
		{
			...
			public void run()
			{
				try
				{
					...
					{
						sendMsg(sender,
								t("restoreBackup.success").replace("{0}",
										args[0]));
					}
				}
