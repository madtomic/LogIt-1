...
public final class LogItCommand extends LogItCoreObject implements TabExecutor
{
			...
			String[] args)
	{
				...
				+ hubCommand.getParams().size())
		{
			...
			for (int i = 0; i < params.size(); i++)
			{
				...
				{
					usageParams.append(t("cmdUsage.param").replace("{0}",
							params.get(i)));
				}
				else
				{
					usageParams.append(t("cmdUsage.missingParam").replace(
							"{0}", params.get(i)));
				}
			}
			...
			{
				usageParams.append(t("cmdUsage.optionalParam").replace("{0}",
						helpLine.getOptionalParam()));
			}

			sendMsg(sender, t("cmdUsage").replace("{0}", helpLine.getCommand())
			...
			String label, String[] args)
	{
		...
		if (hubCommand != null)
		{
			...
			{
				hubCommandArgs = Arrays.copyOfRange(args,
						subcommandWords.length, args.length);
			}
			else
			{
	...
	{
		hubCommandSearch: for (Iterator<HubCommand> it = HubCommands.iterator(); it
				.hasNext();)
		{
	 ...
	 */
	private String[] translateHubCommandArgs(HubCommand hubCommand,
			String[] args)
	{
		List<String> params = hubCommand.getParams();
		String[] subcommandWords = hubCommand.getSubcommand().split("\\s+");
		...
		System.arraycopy(args, subcommandWords.length, hubCommandArgs, 0,
				params.size());
