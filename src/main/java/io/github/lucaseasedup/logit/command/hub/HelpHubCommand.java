...
public final class HelpHubCommand extends HubCommand
{
	...
	{
		super("help", new String[] {}, new CommandAccess.Builder()
				.permission("logit.help").playerOnly(false)
				.runningCoreRequired(false).build(),
				new CommandHelpLine.Builder().command("logit help")
						.descriptionLabel("subCmdDesc.help").build());
	}
	...
	public void execute(CommandSender sender, String[] args)
	{
		for (Iterator<HubCommand> it = HubCommands.iterator(); it.hasNext();)
		{
			...
			if ((!(sender instanceof Player) || sender.hasPermission(hubCommand
					.getPermission()))
					&& !(hubCommand.isPlayerOnly() && !(sender instanceof Player))
					&& (getCore().isStarted() || !hubCommand
							.isRunningCoreRequired()))
			{
				...
				String description = t(hubCommand.getHelpLine()
						.getDescriptionLabel());
