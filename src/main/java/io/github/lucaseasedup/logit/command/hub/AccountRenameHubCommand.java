...
public final class AccountRenameHubCommand extends HubCommand
{
	public AccountRenameHubCommand()
	{
		super("account rename", new String[] { "username", "newUsername" },
				new CommandAccess.Builder().permission("logit.account.rename")
						.playerOnly(false).runningCoreRequired(true).build(),
				new CommandHelpLine.Builder().command("logit account rename")
						.descriptionLabel("subCmdDesc.account.rename").build());
	}
	...
	public void execute(final CommandSender sender, final String[] args)
	{
		...
		if (!args[1].matches(getConfig("secret.yml")
				.getString("username.regex")))
		{
		...
		{
			sendMsg(sender,
					t("accountRename.usernameTooShort").replace("{0}",
		...
		{
			sendMsg(sender,
					t("accountRename.usernameTooLong").replace("{0}",
		...
		if (CollectionUtils.containsIgnoreCase(args[1], getConfig("config.yml")
				.getStringList("prohibitedUsernames")))
		{
		...
		sendMsg(sender,
				t("accountRename.confirm")
						.replace("{0}", args[0].toLowerCase()).replace("{1}",
								args[1].toLowerCase()));
		...
		new ConfirmationWizard(sender, "rename", new Runnable()
		{
			...
			public void run()
			{
				...
				sendMsg(sender,
						t("accountRename.success").replace("{0}",
								args[1].toLowerCase()));
			}
		}).createWizard();
	}
