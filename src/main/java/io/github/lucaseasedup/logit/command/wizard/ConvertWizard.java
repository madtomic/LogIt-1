...
public final class ConvertWizard extends Wizard
{
	...
	protected void onMessage(String message)
	{
		...
		else if (getCurrentStep() == Step.ENTER_DBTYPE)
		{
			...
			{
				sendMessage(t("wizard.convert.unknownStorageType").replace(
						"{0}", message));
			...
			else
			{
				...
				sendMessage(t("wizard.convert.selectedStorageType").replace(
						"{0}", message));
		...
		else if (getCurrentStep() == Step.ENTER_FILENAME)
		{
			...
			sendMessage(t("wizard.convert.enteredFilename").replace("{0}",
					message));
		...
		else if (getCurrentStep() == Step.ENTER_HOST)
		{
			...
			sendMessage(t("wizard.convert.enteredHost").replace("{0}", message));
		...
		else if (getCurrentStep() == Step.ENTER_USER)
		{
			...
			sendMessage(t("wizard.convert.enteredUser").replace("{0}", message));
		...
		else if (getCurrentStep() == Step.ENTER_PASSWORD)
		{
			...
			sendMessage(t("wizard.convert.enteredPassword").replace("{0}",
					message.replaceAll(".", "*")));
		...
		else if (getCurrentStep() == Step.ENTER_DATABASE)
		{
			...
			sendMessage(t("wizard.convert.enteredDatabaseName").replace("{0}",
					message));
		...
		else if (getCurrentStep() == Step.ENTER_TABLE)
		{
			...
			sendMessage(t("wizard.convert.enteredUnit").replace("{0}", message));
		...
		else if (getCurrentStep() == Step.FINISH_CHOICE)
		{
			if ("proceed".equals(message))
			{
				...
				getConfig("config.yml").set(
						"storage.accounts.leading.storageType", dbtype);

				switch (dbtype)
				{
					case "sqlite":
						getConfig("config.yml").set(
								"storage.accounts.leading.sqlite.filename",
								filename);
						break;

					case "h2":
						getConfig("config.yml").set(
								"storage.accounts.leading.h2.filename",
								filename);
						break;

					case "mysql":
						getConfig("config.yml").set(
								"storage.accounts.leading.mysql.host", host);
						getConfig("config.yml").set(
								"storage.accounts.leading.mysql.user", user);
						getConfig("config.yml").set(
								"storage.accounts.leading.mysql.password",
								password);
						getConfig("config.yml").set(
								"storage.accounts.leading.mysql.database",
								database);
						break;
				}

				getConfig("config.yml").set("storage.accounts.leading.unit",
						table);
	...
	public static enum Step
	{
		...
		ENTER_DBTYPE, ENTER_FILENAME, ENTER_HOST, ENTER_USER, ENTER_PASSWORD, ENTER_DATABASE, ENTER_TABLE, COPY_OR_LEAVE,
