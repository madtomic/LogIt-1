		...
		TabExecutor
{
			...
			String[] args)
	{
		...
		{
			if (player != null
					&& !player.hasPermission("logit.changeemail.others"))
			{
			...
			if (args.length < 3)
			{
				sendMsg(sender, t("paramMissing").replace("{0}", "newemail"));
			...
			sendMsg(args[1],
					t("changeEmail.success.self").replace("{0}",
							args[2].toLowerCase()));
			sendMsg(sender,
					t("changeEmail.success.others").replace("{0}",
							PlayerUtils.getPlayerRealName(args[1])).replace(
							"{1}", args[2].toLowerCase()));
		}
		else if (args.length <= 1)
		{
			...
			if (args.length < 1)
			{
				sendMsg(player, t("paramMissing").replace("{0}", "newemail"));

				return true;
			}

			if (getCooldownManager().isCooldownActive(player,
					LogItCooldowns.CHANGEEMAIL))
			{
			...
			Account account = getAccountManager().selectAccount(
					player.getName(), Arrays.asList(keys().username()));
			...
			int accountsWithEmail = getAccountManager().selectAccounts(
					...
					new SelectorCondition(keys().email(), Infix.EQUALS, args[0]
							.toLowerCase())).size();

			if (accountsWithEmail >= getConfig("config.yml").getInt(
					"accountsPerEmail"))
			{
