...
public final class AcclockCommand extends BukkitLogItCoreObject implements
		TabExecutor
{
	@Override
			...
			String[] args)
	{
		...
		if (args.length <= 1)
		{
			...
			if (args.length < 1)
			{
				sendMsg(sender, t("paramMissing").replace("{0}", "username"));
			...
			Account account = getAccountManager().selectAccount(args[0],
					Arrays.asList(keys().username(), keys().is_locked()));

			if (account == null)
			{
				sendMsg(sender,
						t("notRegistered.others").replace("{0}", args[0]));
			...
			{
				PlayerUtils.getPlayer(args[0]).kickPlayer(
						t("acclock.success.self"));
			}

			sendMsg(sender,
					t("acclock.success.others").replace("{0}",
