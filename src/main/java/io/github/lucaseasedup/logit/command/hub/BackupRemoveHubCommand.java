...
public final class BackupRemoveHubCommand extends HubCommand
{
	public BackupRemoveHubCommand()
	{
		super("backup remove", new String[] { "amount" },
				new CommandAccess.Builder().permission("logit.backup.remove")
						.playerOnly(false).runningCoreRequired(true).build(),
				new CommandHelpLine.Builder().command("logit backup remove")
						.descriptionLabel("subCmdDesc.backup.remove").build());
	}
	...
	public void execute(final CommandSender sender, final String[] args)
	{
		...
		{
			sendMsg(sender, t("invalidParam").replace("{0}", "amount"));
		...
		int manualRemoveLimit = getConfig("config.yml").getInt(
				"backup.manualRemoveLimit");
		...
		if (amount > manualRemoveLimit)
		{
			...
			sendMsg(sender,
					t("removeBackups.confirm.limitedAmount").replace("{0}",
							String.valueOf(limitedAmount)));
		}
		else
		{
			...
			sendMsg(sender,
					t("removeBackups.confirm.amount").replace("{0}",
							String.valueOf(limitedAmount)));
		}
		...
		try
		{
			firstBackupDate = getBackupManager().parseBackupFilename(
					firstBackup.getName());
		}
		...
		{
			sendMsg(sender,
					t("removeBackups.confirm.date.single").replace(
							"{0}",
							(firstBackupDate != null) ? firstBackupDate
									.toString() : "?"));
		}
		else
		{
			...
			try
			{
				lastBackupDate = getBackupManager().parseBackupFilename(
						lastBackup.getName());
			}
			catch (ParseException ex)
			{
			...
			sendMsg(sender,
					t("removeBackups.confirm.date.range.start").replace(
							"{0}",
							(firstBackupDate != null) ? firstBackupDate
									.toString() : "?"));
			sendMsg(sender,
					t("removeBackups.confirm.date.range.end").replace(
							"{0}",
		...
		new ConfirmationWizard(sender, "remove", new Runnable()
		{
			...
			public void run()
			{
				int effectiveAmount = getBackupManager().removeBackups(
						limitedAmount);
