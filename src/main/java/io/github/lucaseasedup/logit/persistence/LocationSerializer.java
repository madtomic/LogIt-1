...
@Keys({ @Key(name = "world", constraint = KeyConstraint.NOT_EMPTY),
		...
		PersistenceSerializer
{
	...
	public void serialize(Map<String, String> data, final Player player)
	{
		...
		if (player.isOnline())
		{
			...
			final double dislocationRadius = getConfig("secret.yml").getDouble(
					"locationSerializer.teleportCheck.dislocationRadius");
			...
			new BukkitRunnable()
			{
				...
				public void run()
				{
					...
					else if (!PlayerUtils.isPlayerAt(player,
	...
	private static void serialize(Map<String, String> data,
	...
	{
		return new org.bukkit.Location(Bukkit.getWorld(data.get("world")),
				Double.valueOf(data.get("x")), Double.valueOf(data.get("y")),
				Double.valueOf(data.get("z")), Float.valueOf(data.get("yaw")),
	...
	@Keys({ @Key(name = "world", constraint = KeyConstraint.NOT_EMPTY),
			...
			@Key(name = "yaw", constraint = KeyConstraint.NOT_EMPTY),
			@Key(name = "pitch", constraint = KeyConstraint.NOT_EMPTY), })
	public static final class PlayerlessLocationSerializer implements
			PersistenceSerializer
	{
