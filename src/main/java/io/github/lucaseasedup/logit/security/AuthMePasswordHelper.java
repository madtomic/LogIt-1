...
public final class AuthMePasswordHelper
{
	...
	public static boolean comparePasswordWithHash(String password,
			String hashedPassword, String encryptionMethod)
	{
		if (compareWithEncryptionMethod(password, hashedPassword,
				encryptionMethod))
		{
			return true;
		}
			...
			String hashedPassword, String encryptionMethod)
	{
		try
		{
			switch (encryptionMethod.toUpperCase())
			{
				...
				case "DOUBLEMD5":
				{
					return hashedPassword.equals(SecurityHelper
							.getMd5(SecurityHelper.getMd5(password)));
				}
				case "MD5":
				{
					return hashedPassword.equals(SecurityHelper
				...
				case "SHA256":
				{
					...
					String hash = SecurityHelper.getSha256(SecurityHelper
							.getSha256(password) + line[2]);

					return hashedPassword
							.equals("$SHA$" + line[2] + "$" + hash);
				}
				case "SHA512":
				{
					return hashedPassword.equals(SecurityHelper
				...
				{
					return hashedPassword.equals(SecurityHelper
							.getWhirlpool(password));
				}
				case "XAUTH":
				{
					int saltPos = (password.length() >= hashedPassword.length()) ? hashedPassword
							.length() - 1 : password.length();
					String salt = hashedPassword.substring(saltPos,
							saltPos + 12);
	...
	{
		String hash = SecurityHelper.getWhirlpool(salt + password)
				.toLowerCase();
