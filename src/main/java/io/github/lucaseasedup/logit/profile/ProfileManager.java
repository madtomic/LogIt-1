...
public final class ProfileManager extends LogItCoreObject
{
	...
	public void setProfileString(String playerName, String fieldName,
			String value)
	{
	...
	public void setProfileFloat(String playerName, String fieldName,
			double value)
	{
		if (StringUtils.isBlank(playerName) || StringUtils.isBlank(fieldName))
		{
	...
	private YamlConfiguration getProfileConfiguration(String playerName)
	{
		...
		if (profileFile == null)
		{
			fileCache.put(playerName,
					profileFile = new File(path, playerName.toLowerCase()
							+ ".yml"));
		}
	...
	private void saveProfileConfiguration(String playerName)
	{
		...
		{
			log(Level.WARNING,
					"Could not save profile configuration with player name: "
							+ playerName, ex);
		}
	}
	...
	private Field newField(String fieldName, String definitionString)
	{
		...
		Matcher definitionMatcher = FIELD_DEFINITION_PATTERN
				.matcher(definitionString);
		...
		if (definitionMatcher.find())
		{
			...
			switch (type)
			{
				case "STRING":
				{
					Matcher rangeMatcher = INTEGER_RANGE_PATTERN
							.matcher(arguments);
				...
				case "FLOAT":
				{
					Matcher rangeMatcher = FLOAT_RANGE_PATTERN
							.matcher(arguments);
				...
				case "SET":
					return new SetField(fieldName, Arrays.asList(arguments
							.split("(?<!\\\\),")));
	...
	private final static Pattern FIELD_DEFINITION_PATTERN = Pattern
			.compile("^\\s*([A-Za-z_0-9]+)" // Type name
					+ "\\s*\\[(.*)\\]\\s*$"); // Constraint arguments

	private final static Pattern INTEGER_RANGE_PATTERN = Pattern
			.compile("^\\s*(-?[0-9]+)" // Min
					+ "\\s*\\.\\.\\." // Range operator
					+ "\\s*(-?[0-9]+)\\s*$"); // Max

	private final static Pattern FLOAT_RANGE_PATTERN = Pattern
			.compile("^\\s*(-?[0-9]+\\.[0-9]+)" // Min
					+ "\\s*\\.\\.\\." // Range operator
					+ "\\s*(-?[0-9]+\\.[0-9]+)\\s*$"); // Max
